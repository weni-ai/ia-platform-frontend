  ...
  >
    <div class="tutorial">
      ...
      <BNotification
        ...
        :duration="4000"
        :closable="false"
        :autoClose="autoCloseNotification"
...
export default {
  ...
  computed: {
    ...
    computedList() {
      return this.list.map((item) => ({
        ...item,
        active:
          item.label === 'create_account' ||
          (item.label === 'create_intelligence' &&
            this.repositoryTutorial !== null)
            ? true
            : this.finished[item.label] === 'finished',
      }));
    },
  },
  watch: {
    open() {
      if (this.open === true) {
        ...
        if (this.getCheckRepositoryTrain) {
          this.setNotificationAlert(
            'is-warning',
            false,
            this.$t('webapp.tutorial.training_wait'),
          );
  ...
  methods: {
          ...
          if (this.repositoryStatus.count !== 0) {
            if (this.repositoryStatus.results[0].status === 2) {
              ...
              this.setNotificationAlert(
                'is-success',
                true,
                this.$t('webapp.tutorial.training_success'),
              );
              return;
            }
            if (this.repositoryStatus.results[0].status === 3) {
              ...
              this.setNotificationAlert(
                'is-danger',
                true,
                this.$t('webapp.tutorial.training_error'),
              );
            }
          } else {
            clearInterval(refreshStatus);
    ...
    goToTutorial(name, target) {
      ...
      this.$router.push(
        `/dashboard/${this.nickname}/${this.repositoryTutorial}/${target}`,
      );
      this.openNotification = false;
    ...
    async startTutorial(name, target, previous) {
      try {
        if (target === 'home' || target === 'new') {
          ...
          await this.getTrainingStatus();
          if (
            this.repositoryStatus.length !== 0 &&
          ...
          this.setNotificationAlert(
            'is-danger',
            true,
            this.$t('webapp.tutorial.alert_message'),
          );
        }
      } catch (error) {
        this.error = error;
