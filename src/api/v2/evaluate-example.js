...
export default {
  new(
    ...
    return request.$http.post('/v2/repository/evaluate/', {
      repository,
  ...
  update(repository, version, text, language, entities, intent, id) {
    return request.$http.patch(`/v2/repository/evaluate/${id}/`, {
      repository,
  ...
  delete(exampleId, repositoryUuid) {
    return request.$http.delete(
      `/v2/repository/evaluate/${exampleId}/?repository_uuid=${repositoryUuid}`,
    );
  },
  ...
  getResultsData(repositoryUuid, resultId) {
    return request.$http.get(
      `/v2/repository/evaluate/results/${resultId}/?repository_uuid=${repositoryUuid}`,
    );
  },
  getAllResultsLog(repositoryUuid, resultId, page = 1) {
    return request.$http.get(
      `/v2/repository/evaluate/results/${resultId}/?repository_uuid=${repositoryUuid}&page_intent=${page}`,
    );
  },
  ...
  async resultSearch(repositoryUuid, query = {}) {
    ...
    return request.$http.get(
      `/v2/repository/evaluate/results/1/?${queryString}`,
    );
  },
